    ...
    with torch.no_grad():
        ...
        for ddpcc_ckpt in ckpts:
            ...
            for i in range(1, args.frame_count):
                ...
                d1psnr, d2psnr, cd = PSNR(pc_ori, pc_recon, n1, return_all=False, peak_value=peak_value)
                log_string(str(i) + ' ' + str(bpp)[:7] + ' ' + str(d1psnr)[:7] + ' ' + str(d2psnr)[:7] + ' ' + str(cd)[:7] + '\n')
                f1 = recon_f2
            ...
            inter_keys = ['motion_bpp', 'residual_bpp', 'ys2_2_lossless_bpp', 'gpcc_bpp',
                            ...
                            'round_motion', 'round_residual', 'cd']
            ...
            log_string(seqname + ' average cd: ' + str(mean_results['cd_avg']))
