...
results = {
    'basketball': {'bpp': [], 'd1-psnr': []},
    'dancer': {'bpp': [], 'd1-psnr': []},
    'exercise': {'bpp': [], 'd1-psnr': []},
    'model': {'bpp': [], 'd1-psnr': []}
}
        ...
        if flag == 1:
            if len(words) >= 7 and words[5] == 'INFO' and words[3] == 'Model' and words[7].isdigit():
                if 0 <= int(words[7]) < 32:
                    ...
                    psnr_sum += float(words[9])
                    if int(words[7]) == 31:
                        print('result:', count, bpp_sum/count, psnr_sum/count)
                        results[seq_name]['bpp'].append(bpp_sum/count)
                        results[seq_name]['d1-psnr'].append(psnr_sum/count)
                        flag = 0
                        count = 0
